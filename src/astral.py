...
try:
    ...
    raise ImportError(('The astral module requires '
        'the pytz module to be available.'))
...
class City(object):
    ...
    def __init__(self, info=None):
        ...
        :param info: A tuple of information to fill in the city info.
            ...
            See :attr:`timezone` property for a method of obtaining time
            zone names
        """
...
class Astral(object):
    ...
    def solar_depression():
        doc = """The number of degrees the sun must be below the horizon for
        the dawn/dusk calc.
    ...
    def sun_utc(self, date, latitude, longitude):
        ...
        return {'dawn': dawn, 'sunrise': sunrise, \
            'noon': noon, 'sunset': sunset, 'dusk': dusk}
    ...
    def solar_elevation(self, dateandtime, latitude, longitude):
        ...
        zone = -dateandtime.utcoffset().seconds / 3600.0
        utc_datetime = dateandtime.astimezone(pytz.utc)
        timenow = utc_datetime.hour + \
            (utc_datetime.minute / 60.0) + (utc_datetime.second / 3600)
    
        JD = self._julianday(dateandtime.day, 
            dateandtime.month, dateandtime.year)
        t = self._jday_to_jcentury(JD + timenow / 24.0)
        theta = self._sun_declination(t)
        Etime = self._eq_of_time(t)
    ...
    def _hour_angle(self, latitude, solar_dec, solar_depression):
        ...
        HA = (acos(cos(radians(90 + solar_depression)) / (cos(latRad) * \
